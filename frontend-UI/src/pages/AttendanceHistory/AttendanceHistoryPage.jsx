...
export default function AttendanceHistoryPage({ studentMode = false }) {
  ...
  const load = (page = 0, f = filters) => {
    ...
    if (studentMode) {
      /*
       * Students see only their own ledger — the backend scopes
       * everything to the authenticated account.
       */
      studentPortalService
        ...
        .then(setData)
        .catch((e) =>
          setError(e.response?.data?.message || 'Failed to load your attendance records')
        )
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="mb-1 inline-flex items-center gap-2 rounded-full border border-cyan-300/25 bg-cyan-400/10 px-2.5 py-0.5 text-[11px] font-semibold text-cyan-300 shadow-glow-sm">
                  ...
                  {studentMode ? 'My Records' : 'Ledger Logs'}
                </div>

                <h1 className="font-display text-2xl font-bold tracking-tight text-white sm:text-3xl">
                  {studentMode ? 'My Attendance' : 'Attendance History'}
                </h1>

                <p className="mt-0.5 text-xs text-slate-400 sm:text-sm">
                  {studentMode
                    ? 'Your personal attendance timeline — every verified check-in and teacher mark.'
            ...
            <div className="mb-4 flex items-center justify-between border-b border-white/[0.08] pb-3">
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-slate-500">
                    {studentMode
                      ? 'Narrow down your own records by date intervals or status.'
