...
import { AlertCircle, ArrowLeft, CheckCircle2, Mail, User as UserIcon, Camera, X, Trash2 } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  // ---- Photo state (separate from the name/email form above) ----
  const [photoUrl, setPhotoUrl] = useState(null);       // saved photo, from the server
  const [selectedFile, setSelectedFile] = useState(null); // file chosen but not yet uploaded
  const [previewUrl, setPreviewUrl] = useState(null);     // local object URL for that file
  ...
  // Release the local preview object URL whenever it's replaced/unmounted,
  // to avoid leaking memory on repeated photo selections.
  useEffect(() => {
    return () => {
  ...
  const handleFileSelect = (event) => {
    ...
    // Same rules the backend enforces — checked here too so the user
  ...
  const uploadPhoto = async () => {
    if (!selectedFile || photoSaving) return; // prevent duplicate submissions
    setPhotoSaving(true);
    setPhotoError('');
    setPhotoSuccess('');
    try {
      ...
      setPhotoUrl(newUrl);
      setProfilePhotoUrl(newUrl); // updates Navbar immediately, no logout needed
      cancelSelection();
      setPhotoSuccess('Profile photo updated.');
    } catch (err) {
      setPhotoError(err?.response?.data?.message || 'Unable to upload your photo.');
    } finally {
  ...
  const removePhoto = async () => {
    ...
    try {
      ...
      setPhotoError(err?.response?.data?.message || 'Unable to remove your photo.');
    } finally {
      setPhotoSaving(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">View and edit your account details.</p>
          </div>
        </div>
      </div>

      {/* ---------------- Photo section ---------------- */}
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        {photoError && <AlertBox message={photoError} />}
        {photoSuccess && <SuccessBox message={photoSuccess} />}

        <div className="flex flex-col items-center gap-4 sm:flex-row">
          <ProfileAvatar photoUrl={previewUrl || photoUrl} name={fullName || username} size="xl" />
      ...
      {/* ---------------- Existing name/email form — unchanged ---------------- */}
        ...
        {loading ? (
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-slate-400">Forgot-password OTPs are sent to this address.</p>
            </div>
            ...
            >
              {saving ? (
                <>
                  <span className="h-5 w-5 animate-spin rounded-full border-2 border-white/30 border-t-white" /> Saving...
                </>
              ) : (
                <>
