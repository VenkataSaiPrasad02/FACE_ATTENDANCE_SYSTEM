...
from app.services.embedding_store import EmbeddingStore, get_embedding_store
...
def get_embedding_store_dep() -> EmbeddingStore:
    return get_embedding_store()


# ---------------------------------------------------------------------------
# Embedding cache sync (Java -> Python)
# ---------------------------------------------------------------------------

@router.post("/sync", status_code=status.HTTP_200_OK, summary="Sync embedding cache")
async def sync_embeddings(
    request_data: SyncEmbeddingsRequest,
    store: EmbeddingStore = Depends(get_embedding_store_dep),
):
    """
    Called by Java's EmbeddingCacheService.refresh() after the new snapshot
    has been built — on startup, and after register/update/delete. Replaces
    the ENTIRE in-memory matrix; this is a full resync, not an incremental
    update.
    """
    sync_snap = perf.start()

    candidates = [
        {"student_id": c.student_id, "embedding": c.embedding}
        for c in request_data.candidates
    ]
    store.replace_all(candidates)

    perf.stop(f"Sync - embedding store replace_all ({store.size()} candidates)", sync_snap)
    perf.log_info("Embedding store now holds %d candidates", store.size())

    return {"synced": store.size()}
...
async def register_face(
    ...
    # NOTE: this endpoint does NOT push the new embedding into the
    # in-memory EmbeddingStore. Java persists it to MySQL first, then
    # calls EmbeddingCacheService.refresh(), which POSTs the complete
    # updated snapshot to /api/face/sync. That single sync path is the
    # only place the matrix is rebuilt (no duplicate refresh mechanism).
...
@router.post(
    ...
    description=(
        ...
        "detection, and compare it against the in-memory candidate "
        "embeddings held in the EmbeddingStore (populated via /api/face/sync). "
        "The request carries ONLY the probe image — no candidate list. "
        "Attendance business logic is handled entirely by the Java backend."
    ),
)
async def recognize_face(
    ...
    embedding_store: EmbeddingStore = Depends(
        get_embedding_store_dep
    ),
) -> FaceRecognizeResponse:

    logger.info(
        "Face recognition request against %d in-memory candidates",
        embedding_store.size(),
    )
    ...
    if settings_perf_enabled():
        _log_base64_sizes(request_data.image_base64)
    perf.log_info("Candidates currently in EmbeddingStore : %d", embedding_store.size())

    if embedding_store.size() == 0:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=(
                "No candidate embeddings loaded in the recognition service. "
                "Trigger a sync (POST /api/face/sync) first."
            ),
        )
    ...
    # Vectorized comparison against the in-memory matrix. No Python loop.
    # EmbeddingStore.find_best_match() normalizes the probe once, then does
    # `matrix @ probe_vector` + argmax over ALL candidates in one NumPy call.
    recognition_start = time_module.perf_counter()
    recognition_snap = perf.start()
    matched, student_id, confidence = embedding_store.find_best_match(
        probe_embedding, settings.RECOGNITION_THRESHOLD
    )
    perf.stop(
        f"Recognize - embedding comparison ({embedding_store.size()} candidates, vectorized)",
        recognition_snap,
    )
    record_timing(request, "recognition", recognition_start)

    perf.log_info(
        "Match result: matched=%s student_id=%s confidence=%.6f",
